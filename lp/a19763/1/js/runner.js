(function () {
  ...
  window.trackNL = trackNL;

  $('#search-type-VIN-Lookup').on('click', function () {
    trackNL('vin_link_click');
  });

  // Search Boxes GA
  $('.js-search-box').on('click', function () {
    var boxSelection = $(this).data('ga');
    trackNL('Homepage ways to search - ' + boxSelection);
  });
  ...
  var cleanSearchValues = function (mapval) {
    ...
    return removeDiacritics(mapval).trim();
  };
  ...
  //empty allow but if value letter required
  ...
  var $input = $('.bv-input--multi input');

  // Focus and Set form style to active state on page load bc first name field is autofocused. (Desktop Only)
  ...
  var errorCheck = function (e) {
    setTimeout(function () {
      $(e.target).hasClass('error') ? $(e.target).parent().addClass('error') : $(e.target).parent().removeClass('error');
      $input.hasClass('error') ? $input.parent().parent().addClass('errorSpace') : $input.parent().parent().removeClass('errorSpace');
    }, 10);
  }
  ...
  // Set active state for focused input fields for mobile viewport widths
...
(function ($, _) {
  ...
  //Safari Fix
  var initializeReloadCachedPageHandler = function () {
